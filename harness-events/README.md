...
Plan、Node、Approval、Async 和 Checkpoint 事实，未来 Metrics、Audit、Billing 或 UI
可以通过 `EventSubscriber` 订阅，而无需反向耦合执行状态机。

## 公共 API

- `ExecutionEvent`：包含 event ID、名称、plan/node ID、时间、state version、
  trace ID 和不可变 attributes。
...
Approval 和 Async ingress 事件。事件发布由执行层按 best-effort 处理：Publisher 或

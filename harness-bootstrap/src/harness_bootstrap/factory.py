"""FinanceClaw 阶段一默认 Composition Root。"""
...
def build_harness(
    *,
    ...
    """组装阶段一 Harness，但不自动执行插件发现或初始化。

    默认实现使用内存 Registry、显式 ``AllowAllPolicy``、``InMemoryTracer``、
    ``DefaultInvocationContextFactory`` 与 ``LocalPluginProvider``。调用方可以从
    Composition Root 替换这些实现，而无需修改 Runtime 或业务插件。
    """

...
2026-09-07 实施决议更新：用户确认将新发布的解读正文改为自然语言／Markdown，移除 LLM
JSON 解析、逐条事实引用硬校验及格式修复重试，温度不变。外壳仍由代码装配，保留输入、盘面身份、
权限与预算检查。详见[文本解读热修复](./Stage-7-文本解读热修复-实施与验证.md)；
本文原 `ZiweiAgentResult@1` 与结构化 finalization 条款只继续适用于冻结的旧发布。

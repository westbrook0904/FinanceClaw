...
执行中，AgentFactory 统一通过 LangGraph `custom` 流发送根 Agent 和 Worker 的工具开始、完成、失败、等待回答与取消状态。原生运行订阅 `messages-tuple` + `custom` 并开启子图事件和断点续传；API 将工具名称和状态投影到同一任务卡，与正文预览共用持久游标、递增卡片序号和通知投递。开始事件无需等待工具返回，后续约每秒合并刷新；短调用可能直接显示完成状态。最多保留 24 条工具调用，优先保留执行中的调用；工具参数、原始结果、异常正文和子 Agent 正文不进入卡片。工具重试合并为一次逻辑调用，停止、换轮、静音和交互状态继续阻止迟到事件覆盖当前卡片。

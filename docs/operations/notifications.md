...
只有统一 API 业务事务提交后才确认受理。处理中显示加载动图和状态文案；授权信息、撤销授权和停止按钮位于默认折叠的“任务选项”。动画复用飞书官方插件的加载图标，由客户端播放，不以重复发送消息模拟进度。等待回答时移除动画，突出问题和表单。停止请求先关闭后续派发和待答交互，卡片显示“正在停止”；远端确认结束后显示“已停止”。撤权封闭后续权限，仍允许观察已经产生的结果。

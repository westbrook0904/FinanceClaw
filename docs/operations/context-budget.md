...
根模型通过 `FinanceClawSettings.context_budget` 装配 `ContextBudget`；上下文策略整体进入根发布指纹。紫微 Worker 的完整提示计数已包含系统/工具，使用 800,000 减输出预留和安全余量的上限，避免二次扣除系统/工具内容。BFF 和 Agent Server 必须使用相同配置并一起重启；已有任务的冻结发布不随热更新改变。

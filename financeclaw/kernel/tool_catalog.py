...
class ToolReleaseCatalogError(LookupError):
...
class ToolReleaseCatalog(Mapping[tuple[str, str], ToolRelease]):
    ...
    使用场景：装配阶段把全部 ToolRelease 注册进目录；Coordination 按
    tool_id+version 精确解析治理声明，或按 tool_id
    取最新版本。目录本身不可变，保证运行期清单稳定可审计。

    Attributes:
        _entries: (tool_id, version) 到 ToolRelease 的不可变映射，
            用 ``MappingProxyType`` 包装防止运行期被篡改。

    """

    def __init__(self, tools: Iterable[ToolRelease]) -> None:
        """用给定的受治理 Tool 构建只读目录。

        Args:
            tools: 待纳入目录的 ToolRelease 集合，允许重复 tool_id
                但不允许重复 (tool_id, version) 组合。

        Raises:
            TypeError: 条目不是 ToolRelease 实例。
            ValueError: 出现重复的 tool_id+version 组合。

        """
        # 1. 逐个校验条目类型并按 (tool_id, version) 建立索引。
        entries: dict[tuple[str, str], ToolRelease] = {}
        for managed in tools:
            if not isinstance(managed, ToolRelease):
                raise TypeError("ToolReleaseCatalog entries must be ToolRelease values")
    ...
    def __getitem__(self, key: tuple[str, str]) -> ToolRelease:
    ...
    def resolve(self, tool_id: str, version: str | None = None) -> ToolRelease:
        ...
        Raises:
            ToolReleaseCatalogError: tool_id 未知，或指定的 version 不存在。
        ...
        if version is not None:
            try:
                ...
                raise ToolReleaseCatalogError(f"unknown tool version: {tool_id}@{version}") from exc
        ...
        if not candidates:
            raise ToolReleaseCatalogError(f"unknown tool: {tool_id}")
        return max(candidates, key=lambda item: _version_key(item.governance.version))

    def latest(self) -> tuple[ToolRelease, ...]:
        """返回目录中每个 tool_id 的最新版本条目，按 tool_id 排序。

        Returns:
            以 (tool_id, 最新版本) 组织的 ToolRelease 元组，顺序确定可复现。

"""已发布 Workflow 声明的不可变目录；协调端无需加载编译图。"""

from collections.abc import Iterable, Iterator, Mapping
from types import MappingProxyType

from financeclaw.kernel.workflows.models import WorkflowRelease, WorkflowStatus
...
class WorkflowCatalog(Mapping[tuple[str, str], WorkflowRelease]):
    ...
    def __init__(self, definitions: Iterable[WorkflowRelease]) -> None:
        """装配目录并校验登记项。
        ...
        Raises:
            TypeError: 登记项不是 WorkflowRelease。
            ValueError: 同一（workflow_id, version）被登记两次。

        """
        entries: dict[tuple[str, str], WorkflowRelease] = {}
        for definition in definitions:
            if not isinstance(definition, WorkflowRelease):
                raise TypeError("WorkflowCatalog entries must be WorkflowRelease values")
    ...
    def __getitem__(self, key: tuple[str, str]) -> WorkflowRelease:
    ...
    def resolve(self, workflow_id: str, version: str | None = None) -> WorkflowRelease:
        """解析指定工作流的确定版本定义。
    ...
    def published(self) -> tuple[WorkflowRelease, ...]:

...
class FeishuChannelService:
    ...
    def __init__(
        ...
        *,
        ...
        self.scopes = scopes
        from financeclaw.api.application.feishu_card_actions import FeishuCardActions

        self.card_actions = FeishuCardActions(
            conversation_service.turns,
            app_id=app_id,
    ...
    async def _process_text(
        ...
        authorization_kwargs = {
            "authorization": AuthorizationEvidence(
                ...
                issued_at=current,
                expires_at=current
                + timedelta(seconds=self.conversation_service.turns.settings.turn_grant_seconds),
            )
        }
        ...
        if parsed is None and not normalized.startswith("/"):
            channel_state = await run_sync(
                self.conversation_service.turns.interactions.channel_state,
            ...
            if (
                ...
                and channel_state.get("reason") != "authorization_required"
            ):
                try:
                    parsed = (
                        item["interaction_id"],
            ...
            elif channel_state.get("turn_id"):
                waiting_reason = channel_state.get("reason")
                fallback = (
        ...
        if parsed is not None:
            ...
            await self.conversation_service.turns.interactions.respond(
                identifier,
                response,
                tenant_id=tenant_id,
                subject_id=subject_id,
        ...
        if command in {"/cancel", "/authorize", "/revoke", "/mute"}:
            ...
            if parts[0] == "/mute":
                await run_sync(
                    self.conversation_service.turns.notifications,
                    parts[1],
                    tenant_id=tenant_id,
                    subject_id=subject_id,
                    revoke=True,
            ...
            if parts[0] in {"/authorize", "/revoke"}:
                if parts[0] == "/authorize":
                    result = await self.conversation_service.turns.reauthorize(
                        parts[1],
                        tenant_id=tenant_id,
                        subject_id=subject_id,
                        scopes=self.scopes,
                    ...
                    result = await self.conversation_service.turns.revoke_authorization(
                        parts[1],
                        tenant_id=tenant_id,
                        subject_id=subject_id,
                        command_id=f"feishu:{self.app_id}:{message.message_id}",
            ...
            result = await self.conversation_service.turns.cancel(
                parts[1],
                tenant_id=tenant_id,
                subject_id=subject_id,
                command_id=f"feishu:{self.app_id}:{message.message_id}",

"""API 会话用例：创建/读取 Journal，并通过注入的 API 运行服务提交操作。"""
...
class ConversationService:
    ...
    def __init__(
        ...
        *,
        ...
        """复用 Journal 与发布目录，并显式注入同库的 API 运行服务。"""
    ...
    async def get_or_create_channel_conversation(
        ...
        *,
        ...
        profile = self.agent_profiles.resolve(self.ROOT_AGENT_ID)
        _, conversation, _ = await run_sync(
    ...
    def messages(
        ...
        *,
        ...
        messages = self.repository.list_messages(conversation_id, after=after, limit=limit)
    ...
    async def assistant_content(
        ...
        *,
        ...
        """通过 API 运行服务处理本次请求。"""
        return await self.turns.assistant_content(
            turn_id, tenant_id=tenant_id, subject_id=subject_id
        )
    ...
    async def status(
        ...
        *,
        ...
        """通过 API 运行服务处理本次请求。"""
        return await self.turns.status(
            turn_id,
            tenant_id=tenant_id,
            subject_id=subject_id,
            scopes=scopes,
    ...
    async def cancel(
        self, turn_id: str, *, tenant_id: str, subject_id: str, command_id: str
    ) -> TurnSnapshot:
        """通过 API 运行服务处理本次请求。"""
        return await self.turns.cancel(
            turn_id, tenant_id=tenant_id, subject_id=subject_id, command_id=command_id
        )
    ...
    def assert_owned(self, turn_id: str, *, tenant_id: str, subject_id: str) -> None:
        """通过 API 运行服务处理本次请求。"""
        return self.turns.assert_owned(turn_id, tenant_id=tenant_id, subject_id=subject_id)

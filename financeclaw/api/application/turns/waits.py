...
from financeclaw.api.application.turns.backend import current_messages, interrupts
...
def worker_binding(call, snapshot, releases):
    ...
    binding = {
        ...
        "invocation_id": digest(
            [snapshot["context"]["turn_id"], call["id"], raw, digest(normalized)]
        ),
    }
...
def map_wait(observation, snapshot, releases, settings):
    ...
    if "action_requests" in payload:
        ...
        point = InteractionPoint(
            ...
            required_scope=settings.approval_scope,

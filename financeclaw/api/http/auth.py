...
本模块属于 interfaces（HTTP 协议适配层）。生产 API 使用 OIDC/JWT，
...
class StaticBearerAuthenticator:
    ...
    使用场景：仅限本地开发与测试环境，把配置的 ``api_auth_token``
...
class OIDCJWTAuthenticator:
    ...
    async def authenticate(self, bearer_token: str) -> AuthenticatedPrincipal | None:
        ...
        """
        return await run_sync(self._authenticate_sync, bearer_token)

...
from financeclaw.bff.application.feishu_cards import render_card, response_from_card
...
def toast(message, kind="info"):
    """平台回包与任务执行结果分开表达。"""
    return {"toast": {"type": kind, "content": message}}
...
class FeishuCardActions:
    ...
    async def handle(self, raw):
        """在工作线程完成有界数据库操作，保持 BFF 事件循环可用。"""
        try:
            result = await asyncio.to_thread(self.accept, raw)
    ...
    def accept(self, raw):
        ...
        with self.runs.store.sessions.begin() as session:
            ...
            root = self.runs.store.lock(session, target.run_id)
            ...
            evidence = AuthorizationEvidence(
                ...
                issued_at=current,
                expires_at=current + timedelta(seconds=self.runs.settings.bff_run_grant_seconds),
            )
            if response:

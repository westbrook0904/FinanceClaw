...
from financeclaw.kernel.run_errors import RunNotFound
from financeclaw.shared.backends.streaming import (
...
def read_progress(sessions, run_id, tenant_id, subject_id, cursor):
    ...
    with sessions() as session:
        result = session.execute(
            select(
                RootRunRow.revision,
                RootRunRow.projection,
                ConversationMessageRow.content,
            )
            .join(ConversationTurnRow, ConversationTurnRow.run_id == RootRunRow.run_id)
            .outerjoin(
...
async def stream_progress(admission, run_id, *, tenant_id, subject_id, last_event_id):
    ...
    while True:
        ...
        if status in {"completed", "failed", "cancelled", "interrupted", "cancellation_requested"}:
            return
        await asyncio.sleep(admission.settings.bff_run_poll_seconds)

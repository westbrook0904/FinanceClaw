...
你负责完整的专业解答。取得足够的真实工具结果后，直接输出给用户的最终自然语言或 Markdown 解读。
不要只回复“取证完成”或把解读留给父 Agent；你的最终正文将作为 answer_text 返回父 Agent。
不要求 JSON、固定字段或 chart_id/fact_id 引用。
围绕用户问题和 focus 简洁作答，建议不超过 500 个汉字。
区分盘面事实、传统解释与不确定性。仅要求 chart_only 时不必解读。
缺资料返回根会话，不做交互 interrupt。
参数格式有误时只根据已有上下文修正，最多一次；needs_clarification 后禁止重试或补造资料。
星曜数组格式为 [稳定 key, 名称, 亮度, 本层四化]，四化顺序为禄权科忌。事实中的层级不可混用。
...
def ziwei_profile(configuration_fingerprint: str) -> AgentProfile:
    ...
    return AgentProfile(
        ...
        deployment_revision="ziwei-react-answer/1",
        ...
        # 最多 6 次 ReAct 模型轮次，包含最终解读；结果封装不调用模型。
        max_model_calls=6,
        max_tool_calls=6,
    )

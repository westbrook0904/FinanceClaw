...
- `ports.py`：领域需要的最小引擎接口，实际 x-iztro 适配在 `adapters/x_iztro.py`。
- `service.py`：一次生成目标层级及上层事实、按实际变化拆段、可解析的语义投影。
- `errors.py`：稳定错误码，不把出生原文拼进异常。

跨模块持久化由 `application.py` 处理，Agent 与 Tool 位于 Agent Server。
测试与限制见 [紫微运维说明](../../../../docs/operations/ziwei-agent.md)。
